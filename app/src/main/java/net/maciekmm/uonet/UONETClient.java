...
public class UONETClient {
    ...
    public <T> T doRequest(String baseURL, RequestBase<T> req) throws MalformedURLException, UONETException {
        ...
        try {
            ...
            if (responseCode < 200 || responseCode > 206) {
                if (responseCode == HttpURLConnection.HTTP_MOVED_TEMP || responseCode == HttpURLConnection.HTTP_MOVED_PERM || responseCode == HttpURLConnection.HTTP_SEE_OTHER) {
                    ...
                    if (newLocation != null) {
                        return doRequest(newLocation, req);
                    } else {
                        throw new UONETException("Redirection received, but no 'Location' header provided.");
                    }
                } else {
        ...
        try (InputStream stream = connection.getInputStream()) {
            // Check if there is content to map
            if (stream.available() > 0) {
                T resp = mapper.readValue(stream, req.getResponseClass());
                return resp;
            } else {
                throw new UONETException("No content to map due to end-of-input");
            }
        } catch (IOException e) {
